...
export default function BookingsPage() {
  ...
  const desktopNavItems = getBookingsDesktopNavItems(t);
  const mobileNavItems = getBookingsMobileNavItems(t);
  ...
  const currencyLabel = trip.defaultCurrencies.join(" / ");
  const sosCountries = emergencyCountriesForVisual(destinationVisual);

  return (
    <div className="stitch-today-page stitch-bookings-page">
      ...
      <nav className="stitch-side-nav" aria-label={t("nav.bookings")}>
      ...
      <main className="stitch-main-canvas stitch-bookings-main">
        <section className="stitch-trip-heading stitch-bookings-heading" aria-label={t("page.bookings.eyebrow")}>
          <div>
            <h1>{trip.name}</h1>
            <p>{tripDetailLabel}</p>
          </div>
        ...
        <div className="stitch-dashboard-grid stitch-bookings-grid">
          <div className="stitch-main-stack stitch-bookings-stack">
            <section className="bookings-masthead stitch-card" aria-labelledby="bookings-page-title">
              <div className="bookings-masthead__copy">
                <h2 id="bookings-page-title">{t("page.bookings.title")}</h2>
      ...
      <nav className="stitch-bottom-nav" aria-label={t("nav.bookings")}>
...
function getBookingsDesktopNavItems(t: TFunction) {
  return [
    { href: "/", icon: "home", label: t("nav.home"), active: false },
    { href: "/itinerary", icon: "event_note", label: t("nav.plan"), active: false },
    { href: "/bookings", icon: "confirmation_number", label: t("nav.book"), active: true },
...
function getBookingsMobileNavItems(t: TFunction) {
  return [
