...
- Do not run setup generation against the current production `active-trip` during controlled pilot review unless the user explicitly approves resetting existing production workspace data; the current business tables still lack `trip_id`, and setup generation resets active-trip tables.
